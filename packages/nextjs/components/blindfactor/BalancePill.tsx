"use client";

import { useTokenBalance } from "~~/hooks/blindfactor/useTokenBalance";

export const BalancePill = () => {
  const { balance, isDecrypting, isRevealed, reveal, isConnected } = useTokenBalance();

  if (!isConnected) return null;

  const label = isDecrypting ? "..." : balance !== null ? balance : isRevealed ? "..." : "View";

  return (
    <button
      type="button"
      onClick={reveal}
      disabled={isDecrypting}
      title="Click to decrypt your bfUSD balance"
      className="hidden h-8 items-center gap-1.5 rounded-full border border-[#ede4d5] bg-[#fffcf7] px-3 text-xs font-semibold text-[#1a1208] transition-colors hover:border-[#c45c2e] hover:text-[#c45c2e] sm:flex"
    >
      <span className="text-[10px] font-bold uppercase tracking-[0.1em] text-[#9a8a7e]">bfUSD</span>
      <span className="ml-0.5">{label}</span>
    </button>
  );
};
